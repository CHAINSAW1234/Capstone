...
public class XRRotatableGrabInteractable : UnityEngine.XR.Interaction.Toolkit.Interactables.XRGrabInteractable
{
    ...
    [SerializeField] private float rotationSpeed = 90f; // 초당 회전 속도 (도 단위)
    ...
    private void EnsureRigidbody()
    {
        ...
        if (rb == null)
        {
            ...
            // 사진에 표시된 설정값 적용
            rb.mass = 1f;                        // Mass
            rb.linearDamping = 0f;                        // Linear Damping (Drag)
            rb.angularDamping = 0.05f;              // Angular Damping
            rb.useGravity = false;               // Use Gravity
            rb.isKinematic = false;              // Is Kinematic
            rb.interpolation = RigidbodyInterpolation.None; // Interpolate
            rb.collisionDetectionMode = CollisionDetectionMode.Discrete; // Collision Detection
            rb.constraints = RigidbodyConstraints.None; // Constraints (기본값: None)
        }
    }
    ...
    private void RotateObject(float deltaTime)
    {
        ...
        Quaternion horizontalRot = Quaternion.AngleAxis(horizontalRotation, transform.up);    // Y축 기준
        Quaternion verticalRot = Quaternion.AngleAxis(verticalRotation, transform.right);    // X축 기준
